...
export const filterTasks = (
    tasks: Task[],
    filters: TasksFormFilters
): Task[] => {
    const { tagIds: filterTagIds } = filters;

    if (tasks?.length > 0) {
        return tasks.filter(task =>
            filterTagIds.every(filterTagId => task.tagIds.includes(filterTagId))
        );
    }
...
export const getIsTaskUrgent = (tagIds: TagId[]) =>
    tagIds.includes(URGENT_TASK_ID);
